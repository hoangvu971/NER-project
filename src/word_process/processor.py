...
class BaseProcessor(ABC):
    ...
    def _check_and_set_special_tokens(self, tokenizer):
        ...
        if tokenizer.unk_token is None:
            ...
            warnings.warn(
                f"The tokenizer is missing an 'unk_token'. Setting default '{default_unk_token}'.", UserWarning
            )
        ...
        if tokenizer.pad_token is None:
            ...
            warnings.warn(
                f"The tokenizer is missing a 'pad_token'. Setting default '{default_pad_token}'.", UserWarning
            )
        ...
        for token in text:
            if not token.strip():
                ...
                redecoded = self.transformer_tokenizer.decode(
                    self.transformer_tokenizer.encode(token), skip_special_tokens=True
                )
                if token != redecoded:
    ...
    def tokenize_inputs(self, texts, entities):
        ...
        tokenized_inputs = self.transformer_tokenizer(
            input_texts, is_split_into_words=True, return_tensors="pt", truncation=True, padding="longest"
        )
    ...
    def batch_generate_class_mappings(
        self, batch_list: List[Dict], negatives: List[str] = None
    ...
    def collate_raw_batch(
        ...
        class_to_ids: Dict = None,
        id_to_classes: Dict = None,
    ) -> Dict:
